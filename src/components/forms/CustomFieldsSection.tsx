...
interface CustomFieldsSectionProps {
  ...
  onChange: (values: Record<string, string | number | boolean | string[]>) => void;
      ...
      case 'text':
        return (
          <ValidationInput
            key={field.id}
            label={field.name}
            value={typeof values[field.id] === 'boolean' ? '' : (values[field.id] as string | number | readonly string[] | undefined) || ''}
                ...
                {field.options?.map((option) => (
                  <label key={option} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => {
                        const currentValues = Array.isArray(values[field.id]) ? values[field.id] as string[] : [];
